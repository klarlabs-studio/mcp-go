...
- **npm trusted publishing (OIDC): `registry-url` IS REQUIRED, and `ENEEDAUTH` vs `404` tells you which side to fix (fleet — RESOLVED for warden 2026-07-11, v0.18.16).** The whole two-session warden saga came down to a WRONG belief (previously recorded here): that `setup-node`'s `registry-url` conflicts with OIDC. It does NOT — the **official npm docs example sets `registry-url: "https://registry.npmjs.org"`**, and that input is what points npm at npmjs so its OIDC detection engages. **Omitting it → npm never attempts OIDC → `ENEEDAUTH`.** Correct GitHub Actions setup: `setup-node@v6` **with** `registry-url` + `node-version: 24` (bundled npm is ≥ 11.5.1 — do NOT force-install/pin a version; both 11.x and 12.x were red herrings), `permissions: id-token: write` (workflow-level so all jobs inherit), `npm publish --provenance`, and **do NOT set `NODE_AUTH_TOKEN` at all** (setting it to `""` empties the .npmrc token and reverts to `ENEEDAUTH`; setup-node's default placeholder is harmless and OIDC is used regardless). Per-platform `package.json` must carry a `repository.url` matching the GitHub repo. **The decoder ring: `ENEEDAUTH` = npm never tried OIDC (WORKFLOW bug — fix `registry-url`/token/version); `404` "not found or no permission" = OIDC WORKED but the npmjs trusted-publisher config doesn't match (ACCOUNT side — org casing must exactly match the URL slug, workflow filename exact incl. `.yml`, Environment blank, "npm publish" allowed-action selected, and the publisher must exist per-package incl. per-platform scoped packages).** Also: confirm published package NAMES from the job log before declaring failure — warden's used npm os/cpu naming (`warden-linux-x64`, `warden-win32-x64`), not `amd64`/`windows`, and checking the wrong names looked like a persistent 404.
